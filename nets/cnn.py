...
# Everything is the same as before, except that the constructor has no arguments 'in_channels' and 'num_classes', 
# but extracts these quantities from the dataloader using the following function. 
...
def simple_cnn(dataloader, dropout_const = 1.0):
...
def make_resnet9(dataloader):
  return ResNet9(dataloader)
...
def make_resnet18v2(dataloader):
    in_channels, num_classes = in_out_dim(dataloader)
    return _make_resnet18v2(num_classes)

def _make_resnet18v2(num_classes):
